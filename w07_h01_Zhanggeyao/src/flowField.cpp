...
void flowField::setNoise(float _x, float _y) {
    for (int y = 0; y < fieldHeight; y++) {
        for (int x = 0; x < fieldWidth; x++) {
			...
			if(ofDist(particles[i].pos.x,particles[i].pos.y,ofGetMouseX(),ofGetMouseY())<200){
                float dist = ofDist(particles[i].pos.x,particles[i].pos.y,ofGetMouseX(),ofGetMouseY());
				ofMap(dist, 0, 200, 0, TWO_PI);

				float mouseDeg = ofRadToDeg(dist);
				 field[index].set(ofVec2f(cos(mouseDeg) * ofGetMouseX()/500, sin(mouseDeg) * ofGetMouseY()/500));
			}
			}
        }
    }
