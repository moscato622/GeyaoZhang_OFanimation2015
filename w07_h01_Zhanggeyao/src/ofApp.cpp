...
void ofApp::draw(){
	...
	for (int i = 0; i < particles.size(); i++){
			...
			ofLine(particles[i].pos.x,particles[i].pos.y,ofGetMouseX(),ofGetMouseY());
	...
	*/
		...
		for( int j = 0; j < particles.size(); i++){
		
			if(ofDist(particles[i].pos.x,particles[i].pos.y,particles[j].pos.x,particles[j].pos.y)<50){
