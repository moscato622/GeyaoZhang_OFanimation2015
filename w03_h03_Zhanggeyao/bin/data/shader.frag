...
float makePoint(float x,float y,float fx,float fy,float sx,float sy,float t){
   float xx=x+sin(t*fx/5)*sx*abs(cos(sin(t/10)))*1.5;
   float yy=y+cos(t*fy/5)*sy*abs(cos(sin(t/10)))*1.5;
   return 0.4*vol*1.0/sqrt(xx*xx+yy*yy);;
}
...
vec3 grad(float f){
	vec4 c01=vec4(0.0,0.0,0.0,0.10);
	vec4 c02=vec4(0.0,0.0,0.5,0.50);
	vec4 c03=vec4(0.0,0.0,1.0,0.55);
	vec4 c04=vec4(0.0,1.0,1.0,0.80);
	vec4 c05=vec4(2.0,2.0,2.0,1.00);
