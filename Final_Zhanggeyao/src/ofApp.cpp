...
void ofApp::setup(){
	...
	speed =1;
    flowField.setup(ofGetWidth(), ofGetHeight(),(ofGetWidth()+ofGetHeight())/2,  100.0);
	...
	beats.loadSound("sounds/backgroundMusic.wav");
...
void ofApp::update(){
	...
	float x = cos(ofGetElapsedTimef() *0.5)*100;
	//float y = sin(ofGetElapsedTimef() *0.5) * radius;
    //ofRotate(ofGetElapsedTimef()*1000);
	cam.setPosition(ofGetWidth()/2+x, ofGetHeight()/2,  ofGetHeight()/2);
    cam.lookAt(center, ofVec3f(0,0,1));
	//cam.rotate(ofGetElapsedTimef()*100);
	cam.rotate(ofGetElapsedTimef()*5,ofGetWidth()/2, ofGetHeight()/2,  ofGetHeight()/2);
}
...
//////////////////////////Draw Lines/////////////////////////////
	...
	for (int i = 0; i < particles.size(); i++){
		...
		ofSetColor(255,ofGetElapsedTimef()*ofNoise(ofGetElapsedTimef()));
